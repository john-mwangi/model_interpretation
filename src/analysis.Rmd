...
partial(object = model_rf_wr, 
        pred.var = "clean_age", 
        which.class = "No.continuation", 
        plot = TRUE, 
        plot.engine = "ggplot2", 
        smooth = TRUE) +
  labs(title = "PDP for Age on No Continuation")


partial(object = model_rf_wr, 
        pred.var = "clean_age", 
        which.class = "Continuation.3", 
        plot = TRUE, 
        plot.engine = "ggplot2", 
        smooth = TRUE) +
  labs(title = "PDP for Age on Continuation 3")
...
partial(object = model_rf_wr, 
        ...
        smooth = TRUE) %>% 
  as_tibble() %>% 
  mutate(Clinic = fct_reorder(Clinic, desc(yhat))) %>% 
  ggplot(aes(x = Clinic, y = yhat)) +
  geom_col() +
  coord_flip() +
  labs(title = "PDP for Age on Continuation 3")
...
```{r eval=FALSE, include=FALSE}
plot_pdp <- function(model, continuation_rate, variable){
  ...
  pdp_plots[[1]] <- pdp_plot
  
  return(pdp_plots)
}

pdp_plots <- plot_pdp(model = model_rf_wr, continuation_rate = "Continuation.3", variable = "clean_age")
...
pdp_plots <- plot_pdp(model = model_rf_wr, continuation_rate = "No.continuation", variable = "Clinic")
...
```{r eval=FALSE, include=FALSE}
pdp_tbl <- function(model, continuation_rate, model_vars){
  ...
  pdp_df <-
    ...
    mutate(continuation_rate = {{ continuation_rate }})
}
```
...
pdp_res_tbl <- tibble(.rows = 0)

for (var in seq_along(model_vars)){
  pdp_df <- pdp_tbl(model = model_rf_wr, 
                    continuation_rate = "Continuation.3", 
                    model_vars = model_vars[var])
  pdp_res_tbl <- rbind(pdp_res_tbl, pdp_df)
}
...
pdp_res_tbl %>% 
  group_by(variable) %>% 
  arrange(desc(yhat), .by_group = TRUE) %>% 
...
```{r}
save.image("../outputs/16July.RData")
#load("../outputs/15July.RData")
```
