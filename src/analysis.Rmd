...
data_labeled <-
data_labeled %>% 
  mutate(across(.cols = c("e1demoend1q21","e1end11e2hseq29","e1end11e2hseq29b",
...
demo_vars_cat <-
  demo_vars %>% 
  select(-Population, where(is_character)) %>% 
  colnames()
...
demo_categorical %>% 
  bind_rows(demo_numeric) %>%
  left_join(demo_nulls, by = c("Population", "variable")) %>% 
  left_join(data_dictionary, by = "variable") %>% 
  mutate(Population = str_squish(Population)) %>% 
  mutate(total = case_when(Population == "AGYWs" ~ 198,
                           Population == "Female Sex Workers (FSWs)" ~ 200,
                           TRUE ~ 199)) %>% 
  mutate(percentage = n/total) %>% 
  arrange(variable, Population, desc(percentage)) %>% 
  select(Population, variable, label, category = response, frequency = n, nulls, total, percentage, mean, median) %>% 
  mutate(percentage = scales::percent(x = percentage, accuracy = 0.01))
...
df <-
  segmented_obs %>% 
  count(response = .data[[cat_cols[col]]], name = "frequency") %>% 
...
```{r}
imp_vars_desc <-
  bind_rows(count_df, means_df) %>% 
  rename(category = response) %>% 
  arrange(variable, population, segment)
...
```{r}
write_xlsx(x = socio_demographic_stats,
           path = "../outputs/socio_demographic_stats.xlsx")
```

## Images

```{r}
save.image("../outputs/08Aug.RData")
```
...
load("../inputs/Behavioural Scoring/b_score_model.RData")
load("../outputs/07Aug.RData")
#load("../inputs/Behavioural Scoring/b_score_model.RData")
```


```{r}
#saveRDS(object = model_rf_wr, file = "../inputs/model_rf_wr.rds")
```


```{r}
#model_rf_wr <- readRDS(file = "../inputs/model_rf_wr.rds")
```
