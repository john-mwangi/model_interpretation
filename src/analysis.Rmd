...
data_labeled %>% 
  mutate(e1end11e2hseq32 = str_replace(string = e1end11e2hseq32, 
                                       pattern = "4", 
                                       replacement = "Undecided/ Don’t know"))
...
Expand the criteria for FSW to the entire survey and check for major differences.

Numeric variables: no major differences exist apart from the behavioural score.

```{r}
# Numeric variables - check for differences in the means
data_labeled %>%
  select(-b_score, -b_group) %>% 
  filter(Population=="FSWs") %>% 
  drop_var_labs() %>% 
  left_join(segment_data, by = "Client_ID") %>% 
  ...
  rename(segment_1 = "1",
         segment_2 = "2") %>% 
  mutate(difference = abs(segment_1 - segment_2)) %>% 
  arrange(desc(difference))
```

```{r}
# Categorical variables - check differences in proportions
fsw_categorical <-
data_labeled %>%
  select(-b_score, -b_group, -b_segment) %>% 
  filter(Population=="FSWs") %>% 
...
```{r}
fsw_tbl <- tibble(.rows = 0)

for(cat in seq_along(colnames(fsw_categorical))){
  fsw_df <-
  fsw_categorical %>% 
    count(fsw_categorical[[cat]]) %>% 
    set_names(c("segment","category","n")) %>% 
    mutate(variable = colnames(fsw_categorical)[cat])
  
  fsw_tbl <- rbind(fsw_tbl, fsw_df)
}
...
```{r}
fsw_tbl %>% 
  left_join(personas_sizes %>% 
              filter(Population=="FSWs"), by = "segment") %>% 
  rename(n = n.x,
         size = n.y) %>% 
  mutate(prop = n/size) %>% 
  select(-n, -size, -Population) %>% 
  pivot_wider(names_from = segment,
              values_from = prop) %>% 
  rename(segment_1 = "1",
         segment_2 = "2") %>% 
  replace_na(replace = list(segment_1 = 0, segment_2 = 0)) %>% 
  mutate(difference = abs(segment_1 - segment_2)) %>% 
  arrange(desc(difference)) %>% 
  left_join(data_dictionary_updated, by = "variable") %>% 
  View()
...
Create scores for Likert scales questions.

```{r}
likert_ques <-
persona_analysed %>% 
  filter(type %in% c("Conscientiousness","Peer Influence")) %>% 
  pull(variable)
...
```{r}
likert_func <- function(x){
  case_when(x=="strongly agree" ~ 1,
            x=="agree" ~ 2,
            x=="neutral" ~ 3,
            x=="disagree" ~ 4,
...
peer_influence_df <-
data_labeled %>% 
  select(e1end11e2rand3_q125:e1end11e2rand3_q129) %>% 
  map_df(.x = ., .f = list(str_squish, str_to_lower, likert_func))
...
data_labeled %>% 
  select(careless:happen_happen) %>% 
  map_df(as.numeric)

conscientiousness_df
```

The lower the score the more conscientious the individual.

```{r}
consc_score <-
conscientiousness_df %>% 
  select(any_of(likert_ques)) %>% 
  mutate(consc_score = rowSums(across(everything()))) %>% 
  select(consc_score)

consc_score
```

The lower the score the more the individual is influenced by peers.

```{r}
peer_score <-
peer_influence_df %>% 
  select(any_of(likert_ques)) %>% 
  mutate(peer_score = rowSums(across(everything()))) %>% 
  select(peer_score)
...
data_labeled %>% 
  ...
  left_join(segment_data, by = "Client_ID") %>% 
  select(-population) %>% 
  cbind(consc_score, peer_score) %>% 
  write_xlsx("../../Dashboard/persona_data.xlsx")
```
...
```{r}
save.image("../outputs/24Aug.RData")
```
